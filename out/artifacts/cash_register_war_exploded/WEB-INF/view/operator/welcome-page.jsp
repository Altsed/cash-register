...
<script>
    function validateform(){
        var weight=document.updateQuantityForm.is_weight.value;
        var quantity=document.updateQuantityForm.quantity.value;
        alert("It not weight product, quantity must be whole number");
        if(weight == !"true" && quantity % 1 != 0){
            alert("It not weight product, quantity must be whole number");
            return false;
        }
     }
            ...
            <div class="container-fluid">
                <table class="table table-bordered table-hover" id="table">
                    ...
                    <c:forEach var="receipt" items="${receipt}">
                        Product: ${receipt.key.name}  - Capital: ${receipt.value}
                            ...
                            <td>
                                <form name="updateQuantityForm" method="POST" action="update-quantity" onsubmit="return validateform()">
                                    <input type="number" step="0.01" min="0" name="quantity"/>
                                     <input hidden type="text" name="is_weight" value="${products.isWeight}"/>
                                    <input hidden name="productId" value="${receipt.key.reference}">
                                    <input hidden type="number" name="receipt_id" value="${receipt_id}"/>
                                    <button type="submit" class="btn btn-secondary btn-sm" name="command"
                                            value="stockman.UpdateStock" method="POST" action="update-quantity">
